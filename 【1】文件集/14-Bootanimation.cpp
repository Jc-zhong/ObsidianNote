...
status_t BootAnimation::initTexture(Texture* texture, AssetManager& assets,
    ...
    // Jc-z - 【12-01】 : open 目标文件
    Asset* asset = assets.open(name, Asset::ACCESS_BUFFER);
    if (asset == nullptr)
        return NO_INIT;

    // 将文件转换为 Bitmap ，然后提取其文件信息（宽、高、格式等）
...
bool BootAnimation::android() {
    ...
    SLOGD("%sAnimationShownTiming start time: %" PRId64 "ms", mShuttingDown ? "Shutdown" : "Boot",
            elapsedRealtime());
    // Jc-z - 【12-00】: 初始化纹理 - 这里实际就是在 frameworks/base/core/res/assets/images 目录下
    // 将 android-logo-mask.png 和 android-logo-shine.png 两张图片进行初始化纹理
    // 接着到 initTexture 方法实现 -> L284
    ...
    do {
        ...
        // 融合、绑定 Android logo 图 & 绘制
        glEnable(GL_BLEND);
        glBindTexture(GL_TEXTURE_2D, mAndroid[0].name);
        drawTexturedQuad(xc, yc, mAndroid[0].w, mAndroid[0].h);

        // 通过 SwapBuffers 把数据提交到 Display 画布中，完成绘制一帧的显示
        ...
        // 检车是否开机完成，若完成则退出循环，关闭开机动画，进入到 Launcher
        checkExit();
    } while (!exitPending());
...
// Jc-z - 【13-01】： 此处流程类比于 【12】 笔记中的 openGL 绘制流程
// 这里要注意的是： Font 这个参数已经将 纹理 给准备好了，因此我们需要先追踪这个 Font 是怎么来的
// 此时我们可以搜索 initFont() 函数（其实就在该函数的上面定义了）
...
void BootAnimation::drawClock(const Font& font, const int xPos, const int yPos) {
    ...
    // Jc-z - 【13-00】 : 获取时间
    // tips : 传参过来的 Font 是一个结构体，它包含 FileMap 、 Texture 、 width 、 height 四个属性
    time_t rawtime;
    time(&rawtime);
...
bool BootAnimation::parseAnimationDesc(Animation& animation)  {
    ...
    // // Jc-z - 【14-03】 读取文件，这里就规定了 "desc.txt" 文件
    ...
    for (;;) {
        ...
        int nextReadPos;
        // // Jc-z - 【14-04】 逐行解析 desc.txt 文件的各个字段 
        // <width> <height> <fps>
                        ...
                        if (method == ZipFileRO::kCompressStored) {
                            ...
                            if (map) {
                                ...
                                if (leaf == "audio.wav") {
                                    ...
                                    // // Jc-z - 【14-06】 关键部分，把各部分的动画文件 part0 ~ partx 加载到 part.frames 中
...
bool BootAnimation::movie() {
    ...
    // Jc-z - 【14-07】 ** 最关键的部分 ，加载完动画后，执行动画播放
...
bool BootAnimation::playAnimation(const Animation& animation) {
    ...
    // Jc-z - 【14-08】 ** 这个 大循环 就会对已经初始化好的 Animation 对象一步步进行解析，然后执行对应的操作
    // 通过前面分析得知 Animation 包含 desc.txt 的文件信息数据，各个 part0 ~ partx 的图片纹理数据、 COUNT、PAUSE数据信息等
    for (size_t i=0 ; i<pcount ; i++) {
        ...
        // 这里就是将 zip 包中的图片按照顺序 一帧一帧 绘制完成，流程步骤类似于 openGL 绘制那一套
...
BootAnimation::Animation* BootAnimation::loadAnimation(const String8& fn) {
    ...
    // // Jc-z - 【14-02】 构建 Animation ，用于存储 zip 的文件信息
